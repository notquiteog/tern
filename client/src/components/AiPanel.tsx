...
export function AiPanel({ context, onInsert, onSubject, onClose, defaultMode, getDraft, autoRun }: {
  ...
  async function run(saidInstruction?: string) {
    // Passed in rather than read from state when a dictation fires this
    // itself: setInstruction and this call happen in one tick, and the new
    // value is not on `instruction` until the next render.
    const asked = saidInstruction ?? instruction;
    setBusy(true); setOut(''); setFinal(null); setError(''); thinking.reset();
    abort.current?.abort();
    abort.current = new AbortController();
    try {
      await streamWithWork('ai', '/api/ai/draft', { mode, instruction: asked || undefined, tone, length, accountId: context.accountId ?? null, contactId: context.contactId ?? null, threadKey: context.threadKey ?? null, draft: getDraft() || undefined, subject: context.subject, recipientEmail: context.recipientEmail, recipientName: context.recipientName }, {
        ...
        <>
          ...
          {mode !== 'subject' && mode !== 'polish' && (
            <div className="ai-input">
              ...
              <DictateButton
                className="btn-sm"
                title={mode === 'reply' ? 'Say what the reply should do' : 'Say what this should say'}
                onText={(t) => {
                  const said = appendDictated(instruction, t);
                  setInstruction(said);
                  if (prefs.dictateAutoRun) void run(said);
                }}
              />
          ...
          <div className="row">
            <Button size="sm" variant="ai" icon={busy ? <Loader2 size={14} className="spin" /> : <Sparkles size={14} />} onClick={() => void run()} disabled={busy}>{out ? 'Regenerate' : 'Generate'}</Button>
