...
function ReviewCard({ item, onDecide }: { item: any; onDecide: (id: number, a: 'approve' | 'reject', patch?: any) => Promise<void> }) {
  ...
  return (
    <div className="card">
      ...
      {isReply && item.original && <div className="card mb-8" style={{ padding: 10, background: 'var(--bg)' }}><div className="small muted">In reply to <b>{item.original.subject || '(no subject)'}</b> from {item.original.from?.[0]?.email}</div><div className="small muted clamp-2">{item.original.preview}</div></div>}
      ...
      {edit ? <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}><Editor ref={editor} initialHtml={html.current} minHeight={160} onChange={(h) => { html.current = h; }} /></div> : <SafeHtml className="msg-text" html={isReply ? String(item.body_html).split('<div class="tern-quote"')[0] : item.body_html} />}
