...
pgpRouter.post('/autocrypt/gossip', async (req, res) => {
  ...
  const m = await openEmail(req.user!.id, 'owner', await one<any>('SELECT id, to_addr, cc_addr, from_addr, sent_at, received_at FROM emails WHERE id=$1 AND account_id = ANY($2)', [b.emailId, accountIds]));
