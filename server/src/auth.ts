...
export interface UserRow {
  ...
  passkey_auth?: 'second_factor' | 'passwordless';
}
...
export async function attachUser(req: Request, _res: Response, next: NextFunction): Promise<void> {
  ...
  const row = await one<UserRow & { sid: string; last_seen_at: Date }>(
    `SELECT u.id, u.username, u.display_name, u.password_hash, u.role, u.totp_secret, u.totp_enabled, u.recovery_codes, u.totp_last_step, u.prefs, u.disabled, u.password_changed_at, u.created_at, u.last_login_at, u.avatar_updated_at, u.pgp_fingerprint, u.pgp_auth, u.passkey_auth, s.id AS sid, s.last_seen_at
