...
| AI | `ai/llm.ts`, `ai/prompts.ts`, `ai/models.ts`, `ai/slots.ts` | Streams from Ollama's `/api/chat` or an OpenAI-compatible endpoint. Prompts are built from the mode plus the thread, recipient facts and draft. Model tiers by RAM are shared with `install.sh`. `slots.ts` shares the one loaded model out: never more generations at once than Ollama has parallel slots, one slot kept for whoever is waiting at a composer, one interactive generation per person. |
